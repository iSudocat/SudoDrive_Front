...
(function () {
    ...
    // v5 签名
    const CosAuth = function (tmpSecretId, tmpSecretKey, token, startTime, expiredTime, method, query, headers, pathname) {

        if (!tmpSecretId) return console.error('missing param tmpSecretId')
        if (!tmpSecretKey) return console.error('missing param tmpSecretKey')
        if (!token) return console.error('missing param x-cos-security-token')
        if (!startTime) return console.error('missing param startTime')
        if (!expiredTime) return console.error('missing param expiredTime')
        if (!method)
            return console.error('missing param method')
        else
            method = method.toLowerCase()
        // 请求的参数，对象形式传入
        if (!query) return console.error('missing param query')     //{'response-content-disposition':'attachment; filename=1.bat'};
        // 请求的Headers，对象形式传入
        if (!headers) return console.error('missing param headers')
        // 对象路径，以"/"开头
        if (!pathname) return console.error('missing param pathname')   //'/c4/81/c4814bb3-ae6e-4c0d-a78f-dc0e4d80ee17.ini';
        ...
        let  qSignTime = startTime + ';' + expiredTime;
        let  qKeyTime = startTime + ';' + expiredTime;
        ...
        // 步骤一：计算 SignKey
        let  signKey = CryptoJS.HmacSHA1(qKeyTime, tmpSecretKey).toString();
